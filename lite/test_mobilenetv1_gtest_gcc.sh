...
source_model_dir="/workspace/Paddle-Lite/build.self.lite.android.armv7.gcc.opencl/third_party/install/mobilenet_v1"
model_dir="/data/local/tmp/opencl/models/mobilenetv1_project/"
...
adb push build.self.lite.android.armv7.gcc.opencl/lite/api/${testname} /data/local/tmp/opencl/${testname}
adb shell chmod +x /data/local/tmp/opencl/${testname}
cmd="export GLOG_v=1; /data/local/tmp/opencl/${testname} --model_dir=${model_dir} -N=1 -C=3 -H=224 -W=224"
echo ${cmd}
adb shell ${cmd}
