...
source_model_dir="/Users/xiebaiyuan/workspace/github/Paddle-Lite-Vulkan-Dev/baidu_map_cheji/ar_cheji_day/"
model_dir="/data/local/tmp/vulkan/models/ar_cheji_day/"
...
#    python3 ./lite/tools/cmake_tools/gen_opencl_code.py ./lite/backends/opencl/cl_kernel ./lite/backends/opencl/opencl_kernels_source.cc
    cd build.lite.android.armv7.clang
    make $testname -j$4
    cd -
fi
...
cmd="export LD_LIBRARY_PATH=/data/local/tmp/opencl/ ; export GLOG_v=0; /data/local/tmp/opencl/${testname} --model_dir=${model_dir} -N=1 -C=3 -H=192 -W=384 -warmup=10 -repeats=30"
echo ${cmd}
adb shell ${cmd}
