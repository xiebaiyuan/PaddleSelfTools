...
# source_model_dir="/data/coremodels/lens_nanoyolo/split_model"
source_model_dir="/data/src_fp32_fluid_model/OnlineModel/paddle_yolo_nano_fp32"
model_dir="/data/local/tmp/opencl/models/nanoyolofp32/"
...
adb -s ${device} push build.self.lite.android.armv7.clang.opencl/lite/api/${testname} /data/local/tmp/opencl/${testname}
# adb shell chmod +x /data/local/tmp/opencl/${testname}
cmd="export GLOG_v=3; /data/local/tmp/opencl/${testname} --model_dir=${model_dir} -N=1 -C=3 -H=416 -W=416 -warmup=0 -repeats=2 --optimized_model=/data/local/tmp/nanoyolo_self"
echo ${cmd}
adb -s ${device} shell ${cmd}
adb -s ${device} pull /data/local/tmp/nanoyolo_self.nb
