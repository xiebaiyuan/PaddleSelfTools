...
testname="test_elementwise_image_opencl"
# device_id="f5caa946"
...
if [[ "$with_make" == "true" ]]; then
    ...
    cd build.lite.android.armv8.clang
    make $testname -j$6
    cd -
fi
...
adb shell mkdir -p /data/local/tmp/opencl/
adb push build.lite.android.armv8.clang/lite/kernels/opencl/${testname} /data/local/tmp/opencl/${testname}
# adb shell chmod +x /data/local/tmp/opencl/${testname}
adb shell "export GLOG_v=5; /data/local/tmp/opencl/${testname}"
